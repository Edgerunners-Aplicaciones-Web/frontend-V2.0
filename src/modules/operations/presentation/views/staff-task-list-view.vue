    ...
    <div class="card">
      ...
      <pv-column header="" headerStyle="width: 4rem" bodyClass="text-center">
        <template #body="slotProps">
          <!-- ¡AQUÍ ESTÁ LA "TÁCTICA" CORREGIDA! -->
          <!-- 1. :checked (unidireccional) en lugar de v-model -->
          <!-- 2. @click (más fiable que @change) que llama a la "táctica" -->
          <pv-checkbox
              :checked="slotProps.data.isCompleted"
              :binary="true"
              @click="toggleTaskCompletion(slotProps.data)"
      ...
      <pv-column field="roomId" :header="t('staffDashboard.taskRoom')" sortable>
        <template #body="slotProps">
          <!-- getRoomNumber ahora necesita datos de OTRO store -->
...
const propertyStore = usePropertyStore(); // "Pase" del equipo 'Property'

// --- Estado LOCAL (Formulario, Filtros) ---
...
// --- Opciones Computadas (Usan datos de AMBOS Stores) ---
...
const roomOptions = computed(() => {
  // ¡Usa los cuartos del propertyStore!
  return propertyStore.rooms.map(room => ({
...
const tasksWithStatus = computed(() => {
  return store.tasks.map(t => ({ // Usa store.tasks
    ...t,
    // ¡LA "TÁCTICA" CLAVE!
    // No creamos un 'isCompleted' mutable.
    // Simplemente reflejamos el estado del store.
...
// --- Carga Inicial (Llama a AMBOS Stores) ---
onMounted(async () => {
  try {
    ...
    // Carga las TAREAS (de Operations) y los CUARTOS (de Property)
    await Promise.all([
      store.fetchTasks(staffId.value),
      // Usamos 'fetchAllData' que carga rooms Y properties
      propertyStore.fetchAllData()
    ]);
    console.log('StaffTaskList: Datos cargados (Tasks & Rooms)');
...
async function loadTasks() {
  // Esta función ahora solo necesita recargar las tareas
  await store.fetchTasks(staffId.value);
}

// Navegación
function goBack() { router.push({ name: 'staff-dashboard' }); }

// --- Lógica del CRUD (Llama al Store de Operations) ---
function openNew() {
...
function editTask(taskData) {
  // Clonamos la tarea para edición
  task.value = { ...taskData };
  taskDialog.value = true;
}
...
async function saveTask() {
  ...
  try {
    if (task.value.id) {
      // ¡"Pase" al store para ACTUALIZAR!
      await store.updateTask(task.value.id, task.value, staffId.value);
      toast.add({ severity: 'success', summary: t('common.success'), detail: t('tasks.taskUpdated'), life: 3000 });
    } else {
      // "Pase" al store para CREAR!
      const newTaskData = { ...task.value, assignedTo: staffId.value };
      await store.createTask(newTaskData); // Llama al store
      toast.add({ severity: 'success', summary: t('common.success'), detail: t('tasks.taskCreated'), life: 3000 });
    }
    // loadTasks() es llamado DENTRO del store
    hideDialog();
  } catch (error) {
...
async function toggleTaskCompletion(taskData) {
  ...
  try {
    ...
    // El store se encarga de recargar, ¡no necesitamos hacer nada más!
  } catch (error) {
    // (El v-model ya no existe, así que no hay nada que revertir)
...
function confirmDeleteTask(taskData) {
  ...
  confirm.require({
    ...
    accept: async () => {
      // ¡"Pase" al store para BORRAR!
      await store.deleteTask(taskData.id, staffId.value);
...
// --- Helpers Visuales (Usan datos de AMBOS Stores) ---
